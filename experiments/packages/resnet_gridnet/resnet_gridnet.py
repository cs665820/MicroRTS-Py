...
def conv(in_size, out_size, pad=1):
    return layer_init(nn.Conv2d(in_size, out_size, kernel_size=3, stride=1, padding=pad))

class ResBlock(nn.Module):
    def __init__(self, in_size:int, hidden_size:int, out_size:int, relu_func=nn.ReLU()):
        super().__init__()
        self.conv1 = conv(in_size, hidden_size)
        self.conv2 = conv(hidden_size, out_size)
        self.batchnorm1 = nn.BatchNorm2d(hidden_size)
        self.batchnorm2 = nn.BatchNorm2d(out_size)
        self.relu1 = relu_func
        self.relu2 = relu_func
    ...
    def forward(self, x):
        y = self.block(x)
        p = False
        if p: print(f'resblock in shape: {x.shape}')
        if p: print(f'resblock out shape: {y.shape}')
        return x + y # skip connection

class DoubleConeBlock(nn.Module):
    def __init__(self, in_size:int, out_size:int, pad:int, relu_func):
        super().__init__()
        self.conv1 = nn.Conv2d(in_size, 16, kernel_size=2, stride=2, padding=0)
        self.res1 = ResBlock(16, 8, 16, relu_func)
        self.res1 = ResBlock(16, 8, 16, relu_func)
        self.convt1 = nn.ConvTranspose2d(16, out_size, kernel_size=3, stride=2, padding=1, output_padding=1)

    def forward(self, x):
        p = False
        x = self.conv1(x)
        if p: print(f'dc conv1 out: {x.shape}')
        x = self.res1(x)
        if p: print(f'dc res1 out: {x.shape}')
        x = self.convt1(x)
        if p: print(f'dc convt1 out: {x.shape}')
        return x

class QuadConeBlock(nn.Module):
    def __init__(self, in_size:int, out_size:int, pad:int):
        super().__init__()
        self.leftdoublecone = DoubleConeBlock(in_size, out_size, pad, nn.ReLU())
        self.rightdoublecone = DoubleConeBlock(in_size, out_size, pad, nn.RReLU())

    def forward(self, x):
        return x + self.leftdoublecone(x) + self.rightdoublecone(x)

class QuadconeAgent(nn.Module):
    def __init__(self, envs, mapsize=16 * 16):
        super(QuadconeAgent, self).__init__()
        self.envs = envs
        ...
        self.enc_transpose = Transpose((0, 3, 1, 2)) # this puts the board width and height in the first two dimensions, 
        # then the channels in the next dimension,
        # and the individual board in each of the final dimension
        # this matched the (N, C, W, H) input to the layers

        self.enc_conv1 = layer_init(nn.Conv2d(c, 32, kernel_size=3, padding=1))
        self.enc_maxpool1 = nn.MaxPool2d(3, stride=2, padding=1)
        self.enc_relu1 = nn.ReLU()

        self.enc_post_maxpool = nn.MaxPool2d(3, stride=2, padding=1)
        
        self.enc_conv2 = layer_init(nn.Conv2d(32, 64, kernel_size=3, padding=1))
        self.enc_maxpool2 = nn.MaxPool2d(3, stride=2, padding=1)
        self.enc_relu2 = nn.ReLU()

        self.quadcone = QuadConeBlock(32, 32, 0)
...
if __name__ == "__main__":
    ...
    agent = QuadconeAgent(envs).to(device)
    ...
    if args.prod_mode and wandb.run.resumed:
        ...
        agent.load_state_dict(torch.load(f"models/{experiment_name}/agent.pt", map_location=device, weights_only=True))
        agent.eval()
